Opusmodus                                                                   gen-filter-anacrusis    1

gen-filter-anacrusis pitch length &key velocity invert span
[Function]
...
This function is one of a pair with GEN-FILTER-CHANGE. The two functions are at their
most useful as an editor when generating long streams of data; to create phrase boundaries
and turn a single list into a ‘play’ of sublists. One of the problems of using such data for the
composition of music for human performance is partitioning such streams into phrases. Even
instrumental music has to breath and good instrumental music often has a similar temporal
phrase structure as poetry or prose.

(setf mat (rnd-sample 20 '(c4 d4 e4 fs4 gs4) :seed 24))
=> (e4 e4 e4 gs4 e4 gs4 e4 d4 c4 gs4 gs4 fs4 fs4 e4 c4 gs4 gs4 e4 c4 e4)

(gen-filter-anacrusis mat 's)
=> ((-s - e4 gs4 e4 gs4 e4 d4 c4) (-s gs4)
    (-s fs4 e4 c4) (-s gs4 e4 c4 e4))
...
(gen-filter-anacrusis mat 's :invert t)
=> ((s e4 e4) (-s - - - - - - gs4) (-s fs4) (-s - - gs4))
                                                 fi
                                                                                 fi
Opusmodus                                                            gen-filter-anacrusis      2

Examples:
This function is a very ef cient of making effective phrases from long streams of data. You’ll
see that the function mechanism takes the stream and automatically creates sublists. This can
be most valuable should you want to structure a composition around irregular phrase lengths.
...
(gen-filter-anacrusis stream 's)
=> ((s cs4 e4 cs4 d4 e4) (-s eb4 fs4 e4 g4 eb4 d4 cs4)
    (-s fs4 f4 fs4 e4 c4) (-s g4 d4 c4 eb4 g4 c4 cs4 c4 cs4 fs4))

If a single list is required then append the function FLATTEN to the head of the expression.

(flatten (gen-filter-anacrusis stream 's))
=> (s cs4 e4 cs4 d4 e4 -s eb4 fs4 e4 g4 eb4 d4 cs4
    -s fs4 f4 fs4 e4 c4 -s g4 d4 c4 eb4 g4 c4 cs4 c4 cs4 fs4)
...
Opusmodus                                            gen-filter-anacrusis   3
